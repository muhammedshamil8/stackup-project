...
const ProjectTodo = () => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const response = await axiosClient.get(`/projectGetTask.php?userId=${userId}&projectId=${projectId}`);
    ...
    if (isConfirmed) {
      try {
        ...
        if (response.data.status === 1) {
          ...
          const updatedTasks = await axiosClient.get(`/projectGetTask.php?userId=${userId}&projectId=${projectId}`);
